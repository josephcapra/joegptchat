...
export function buildSearchUrl(filters) {
  ...
  // Property types (always include all unless user narrows)
...
export function phraseToFilters(query) {
  ...
  // Geography
  if (query.includes("port st lucie")) filters.city = "Port+St.+Lucie";
  if (query.includes("st lucie county")) { filters.county = "St.+Lucie"; filters.city = "all"; }
  if (query.includes("fort pierce")) filters.city = "Fort+Pierce";
  if (query.includes("hobe sound")) filters.city = "Hobe+Sound";
  if (query.includes("palm city")) filters.city = "Palm+City";
